...
class MaterialSpinnerView(
    ...
    init {
        ...
        context.withStyledAttributes(attrs, R.styleable.MaterialSpinnerView, 0, 0) {
            val str = getString(R.styleable.MaterialSpinnerView_title) ?: ""
            title = str
            ...
            this@MaterialSpinnerView.entries = entries

            val maxLines = getInt(R.styleable.MaterialSpinnerView_android_maxLines, Int.MAX_VALUE)
    ...
    fun popup(): PopupMenu {
        ...
        if (selectedPosition >= 0 && selectedPosition < popup.menu.size) {
            val menuItem = popup.menu[selectedPosition]
    ...
    private fun updateOriginalPositionMenu() {
        ...
        if (originalPosition != selectedPosition &&
            ...
            popup?.menu?.get(originalPosition)?.let { menuItem ->
