...
class LibrarySearchParser(
    private val tokens: List<LibrarySearchLexer.Token>,
) {
    private var index = 0
    ...
    private fun parseAnd(): QueryNode {
        ...
        while (index < tokens.size &&
            peek() !is LibrarySearchLexer.Token.Or &&
            ...
            is LibrarySearchLexer.Token.CompField -> {
                ComparisonField.fromString(nextToken.field)?.let {
                    ComparisonQueryNode(
                        it,
                        nextToken.value,
                        Comparator.fromString(nextToken.comparator)!!,
                        negated,
                    )
